...
- **性能优化**：实现了首屏渲染优化、万级点位渲染支持、防抖和节流优化
...
- **防抖函数（Debounce）**：延迟执行函数，在指定时间内多次调用只执行最后一次
  - 应用于图表更新操作（300ms延迟）
  - 减少数据频繁变化时的重复渲染
  - 提升图表渲染性能

- **节流函数（Throttle）**：限制函数执行频率，在指定时间内只执行一次
  - 地图点击事件节流（500ms）
  - 批量添加标记节流（1000ms）
  - 数据导入导出节流（1000ms）
  - 数据编辑删除节流（500ms）
...
- **防抖（Debounce）**：在用户停止操作一段时间后才执行函数，适用于搜索框输入、窗口调整等场景
- **节流（Throttle）**：限制函数执行频率，在指定时间内只执行一次，适用于滚动事件、地图点击等高频操作
- **优化效果**：减少不必要的函数执行，降低服务器请求频率，提升应用性能和用户体验

### 7. 如何调整防抖和节流的延迟时间？

- 编辑 `src/utils/debounce.js` 文件中的默认延迟时间
- 或在调用函数时传入自定义的延迟时间参数
- 例如：`throttle(func, 2000)` 表示2秒内只执行一次
